...
class LeRobotXarmDataConfig(DataConfigFactory):
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        data_transforms = _transforms.Group(
            inputs=[libero_policy.XarmInputs(action_dim=model_config.action_dim, model_type=model_config.model_type)],
            outputs=[libero_policy.XarmOutputs()],
        )
...
@dataclasses.dataclass(frozen=True)
class XarmPushTDataConfig(DataConfigFactory):
    """
    This config is used to configure transforms that are applied at various parts of the data pipeline.
    For your own dataset, you can copy this class and modify the transforms to match your dataset based on the
    comments below.
    ...
    def create(self, assets_dirs: pathlib.Path, model_config: _model.BaseModelConfig) -> DataConfig:
        ...
        data_transforms = _transforms.Group(
            inputs=[libero_policy.XarmInputs(action_dim=model_config.action_dim, model_type=model_config.model_type)],
            outputs=[libero_policy.XarmPushTOutputs()],
        )
        ...
        # the delta action transform
        if self.use_delta_joint_actions:
            delta_action_mask = _transforms.make_bool_mask(2)
            data_transforms = data_transforms.push(
                inputs=[_transforms.DeltaActions(delta_action_mask)],
                outputs=[_transforms.AbsoluteActions(delta_action_mask)],
            )
...
class TrainConfig:
    ...
    assets_base_dir: str = "./outputs/assets"
...
_CONFIGS = [
    ...
    # the comments below.
    #
    # Fine-tuning on real-to-sim custom tasks.
    #
    TrainConfig(
        name="pi0_lora_insert_rope",
        model=pi0.Pi0Config(paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"),
        data=LeRobotXarmDataConfig(
            repo_id="shashuo0104/xarm7_insert_rope",
            base_config=DataConfig(
                prompt_from_task=True,
    ...
    TrainConfig(
        name="pi0_lora_pack_sloth",
        model=pi0.Pi0Config(paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"),
        data=LeRobotXarmDataConfig(
            repo_id="shashuo0104/xarm7_pack_sloth",
            base_config=DataConfig(
                prompt_from_task=True,
                action_sequence_keys=("action",)
            ),
    ...
    TrainConfig(
        name="pi0_lora_pusht",
        model=pi0.Pi0Config(paligemma_variant="gemma_2b_lora", action_expert_variant="gemma_300m_lora"),
        data=XarmPushTDataConfig(
            repo_id="shashuo0104/xarm7_pusht",
            base_config=DataConfig(
                prompt_from_task=True,
                action_sequence_keys=("action",)
            ),
