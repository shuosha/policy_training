...
from lerobot.common.utils.utils import get_safe_dtype, init_logging
...
class PI0Policy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        rel_action_stats: dict | None = None,
    ):
        """
        Args:
        ...
        if rel_action_stats is None:
            ...
            self.normalize_targets = Normalize(
                config.output_features, config.normalization_mapping, rel_action_stats, rel_actions=True, action_chunk=config.chunk_size
            )
            self.unnormalize_outputs = Unnormalize(
                config.output_features, config.normalization_mapping, rel_action_stats, rel_actions=True, action_chunk=config.chunk_size
            )
    ...
    def _load_as_safetensor(
        ...
        msg = model.load_state_dict(transformed_state_dict, strict=strict) # NOTE: missing original normalization values 
    ...
    def forward(self, batch: dict[str, Tensor], noise=None, time=None) -> tuple[Tensor, dict[str, Tensor]]:
        ...
        images, img_masks = self.prepare_images(batch) # list of [batch_size, C, 224, 224]
        state = self.prepare_state(batch) # pad to [batch_size, 32]
        lang_tokens, lang_masks = self.prepare_language(batch) # (batch_size, 48)
        actions = self.prepare_action(batch) # pad to [batch_size, chunk_size, 32]
