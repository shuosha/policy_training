...
class PI0FAST(nn.Module):
    def __init__(self, config: PI0FASTConfig):
        ...
        if not hasattr(paligemma_config, "ignore_index"):
            paligemma_config.ignore_index = -100
    ...
    def embed_image(self, image: torch.Tensor):
        # manually forward through the vision tower and grab patch embeddings
        # `image` here is already normalized to [-1,1] and sized (B, 3, H, W)
        vision_outputs = self.pi0_paligemma.vision_tower(pixel_values=image)
        # CLIP‐style vision model will return last_hidden_state:
